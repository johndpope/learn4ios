...
public class Divide: BinaryExpression {
    ...
    static func gradients(node: Expression, grad: Expression) -> [Expression] {
        ...
        let leftGrad = (grad / myNode.right).reduceSum(pair.left).reshape(myNode.left.shape)

        let rightDiv2 = (-myNode.left / myNode.right / myNode.right)
        let rightGrad = (grad * rightDiv2).reduceSum(pair.right).reshape(myNode.right.shape)
